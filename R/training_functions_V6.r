...
load_group <- function(g, path.training) {
  ...
  li <- lapply(sample, function(im) {
    data.frame(
      group = g,
      red = as.numeric(imageData(im)[, , 1]),
      green = as.numeric(imageData(im)[, , 2]),
      blue = as.numeric(imageData(im)[, , 3])
    )
  })
  do.call(rbind, li)
}
...
training <- function(path.training, method = "lda", transform = NULL, colormodel = "rgb", mode = "CMD") {
    ...
    writeLOG( path = path.training, create = TRUE, message = "Training run, please wait", detail = "VERSION: 1.0", mode = mode, progress = progress)
    version <- "1.0"
    ...
    ## search for sub-folders in path.training
    writeLOG(path = path.training, message = NULL, detail = paste0("Start training on folder: ",path.training, " 1/6"), mode = mode, value = 1, progress = progress)
    writeLOG(path = path.training, message = NULL, detail = "Load sub-folders 2/7", mode = mode, value = 2, progress = progress)
    ...
    lesionDir <-
      list.dirs(paste0(path.training, "/lesion"), full.names = FALSE)[-1]
    ...
    backgroundDir <-
      list.dirs(paste0(path.training, "/background"), full.names = FALSE)[-1]
    ...
    writeLOG(path = path.training, message = NULL, detail = "Build dataframe with learning 3/7", mode = mode, value = 3, progress = progress)
    ...
    writeLOG(path = path.training, message = NULL, detail = "Build analysis discriminante 4/7", mode = mode, value = 4, progress = progress)
    ...
    writeLOG(path = path.training, message = NULL, detail = "Write output files (csv,jpeg) 5/7", mode = mode, value = 5, progress = progress)
    ...
    jpeg(plotFileTraining1_2, width = 800, height = 800, quality = 100, units = "px")
    if (nbGroups <= 3) {
      ...
      dev.off()
      # Save picture of Discriminent analysis
      jpeg(plotFileTraining1_3, width = 800, height = 800, quality = 100, units = "px")
      g <- plotLDGraph(df4, df4$LD1, "LD1", df4$LD3, "LD3", df4$group, df4$classes.1, backgroundDir, limbDir, lesionDir)
      print(g)
      dev.off()
      # Save picture of Discriminent analysis
      jpeg( plotFileTraining2_3, width = 800, height = 800, quality = 100, units = "px")
    ...
    ## save results
    writeLOG(path = path.training, message = NULL, detail = "Save analysis into R file 6/7", mode = mode, value = 6, progress = progress)
    file.train <-
      paste(path.training, paste0(basename, ".RData"), sep = '/')
    save(train, file = file.train)
    writeLOG(path = path.training, message = NULL, detail = "End of Training 7/7", mode = mode, value = 7, progress = progress)
...
plotLDGraph <- function(df4, x, xname, y, yname, group, class, backgroundDir, limbDir, lesionDir) {
  ...
  g <- ggplot(data = df4, aes( x = x, y = y, colour = group, shape = class)
    ) +
    geom_point() +
    ...
    guides(
      colour = guide_legend(override.aes = list(shape = c(
